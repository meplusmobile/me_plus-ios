...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _handlePurchaseConfirmation(
    int purchaseId,
    int notificationId,
  ) async {
    try {
      ...
      if (purchaseId == 0) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                AppLocalizations.of(context)!.t('purchase_id_missing'),
              ),
      ...
      // Reload profile to update coins immediately with force refresh
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              AppLocalizations.of(context)!.t('purchase_confirmed'),
            ),
            backgroundColor: Colors.green,
          ),
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '${AppLocalizations.of(context)!.t('error')}: ${e.toString()}',
            ),
            backgroundColor: Colors.red,
          ),
        );
  ...
  Future<void> _handlePurchaseRejection(int notificationId) async {
    try {
      // Mark notification as read
      await _markAsRead(notificationId);
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final shouldShowButtons =
              messageEn.toLowerCase().contains('did you receive it') ||
              messageAr.contains('هل استلمتها') ||
              messageAr.contains('هل استلمته');
          ...
          return Dismissible(
            ...
            child: InkWell(
              ...
              child: _buildNotificationCard(
                ...
                time: timeAgo,
                hasButtons: shouldShowButtons,
                isRead: notification.isRead,
                context: context,
                notificationId: notification.id,
                imageUrl: notification.imageUrl,
                purchaseId: notification.purchaseId,
                onYesPressed: shouldShowButtons
                    ? () => _handlePurchaseConfirmation(
                        notification.purchaseId ?? 0,
                        notification.id,
                      )
                    : null,
                onNoPressed: shouldShowButtons
                    ? () => _handlePurchaseRejection(notification.id)
                    : null,
              ),
            ),
          );
