...
class ApiService {
  ...
  Future<Map<String, String>> _getHeaders({
    ...
    // Don't set Content-Type for multipart, let http handle it
    if (!isMultipart) {
  ...
  /// PUT request - NOW SUPPORTS BOTH JSON AND MULTIPART
  Future<ApiResponse> put(
    String path, {
    dynamic data,
    Map<String, dynamic>? queryParameters,
    ...
    try {
      ...
      if (isMultipart && data is Map<String, dynamic>) {
        ...
        final headers = await _getHeaders(withAuth: withAuth, isMultipart: true);
        ...
        debugPrint('📤 [PUT] Sending multipart data: ${request.fields}');
        ...
        final headers = await _getHeaders(withAuth: withAuth);
        final body = data != null ? jsonEncode(data) : null;
        response = await _client.put(uri, headers: headers, body: body).timeout(_timeout);
      }
